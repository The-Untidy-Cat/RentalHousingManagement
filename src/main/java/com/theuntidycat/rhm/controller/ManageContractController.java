...
public class ManageContractController 
{
    ...
    public ResultSet getContractTable()
    {
        ...
        try
        {
            ...
            rs = stmt.executeQuery("SELECT CONTRACT.ID, START_DATE, END_DATE, PRICE_PER_PERIOD, DEPOSIT, TENANT_ID, ROOM_ID, ROOM_STATUS.NAME ,CONTRACT_STATUS.NAME FROM CONTRACT JOIN CONTRACT_STATUS ON CONTRACT.STATUS_ID = CONTRACT_STATUS.ID JOIN ROOM ON CONTRACT.ROOM_ID = ROOM.ID JOIN ROOM_STATUS ON ROOM.STATUS_ID = ROOM_STATUS.ID ORDER BY CONTRACT.ID");
        }
    ...
    public ResultSet loadNewDataContractTable(String contract_id)
    {
        ...
        {
            Statement stmt = con.createStatement();
            rs = stmt.executeQuery("SELECT CONTRACT.ID, START_DATE, END_DATE, PRICE_PER_PERIOD, DEPOSIT, TENANT_ID, ROOM_ID, ROOM_STATUS.NAME ,CONTRACT_STATUS.NAME FROM CONTRACT JOIN CONTRACT_STATUS ON CONTRACT.STATUS_ID = CONTRACT_STATUS.ID JOIN ROOM ON CONTRACT.ROOM_ID = ROOM.ID JOIN ROOM_STATUS ON ROOM.STATUS_ID = ROOM_STATUS.ID WHERE CONTRACT.ID = '"+contract_id+"'");
